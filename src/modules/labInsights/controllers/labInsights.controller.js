...
const getDashboardInsights = async (req, res) => {
  try {
    const userId = req.userId;
    console.log(`\n📊 [getDashboardInsights] Fetching for user: ${userId}`);
...
const generateWeeklyReport = async (req, res) => {
  try {
    const userId = req.userId;
    console.log(`\n📄 [generateWeeklyReport] Generating report for user: ${userId}`);
    ...
    if (report.success) {
      res.json({
        ...
        downloadUrl: `/api/labinsights/lab/weekly-report/download/${report.fileName}`,
